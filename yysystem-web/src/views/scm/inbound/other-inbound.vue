<template>
  <div class="app-container">
    <div class="header-actions">
      <h2>外转入库</h2>
      <el-button v-if="canCreate" type="primary" @click="handleAdd">新增外转入库</el-button>
    </div>
    ...
    <div class="pagination-container">
      <el-pagination v-model:current-page="currentPage" v-model:page-size="pageSize" :page-sizes="[10, 20, 50, 100]"
        :total="total" layout="total, sizes, prev, pager, next, jumper" @size-change="fetchData"
        @current-change="fetchData" />
    </div>

    <!-- 详情弹窗 -->
    <el-dialog v-model="dialogVisible" title="外转入库单详情" width="80%">
      <div v-if="currentInbound">
        ...
        <el-table :data="currentInbound.detailList" border stripe>
          ...
          <el-table-column prop="costAmount" label="金额" width="120" align="right">
            <template #default="scope">
    ...
    <!-- 新增外转入库弹窗 - 选择采购申请表 -->
    <el-dialog v-model="addDialogVisible" title="选择采购申请表创建外转入库" width="70%">
...
import { getInboundList, getInboundById, auditInbound, createTransferFromPurchase, deleteInbound } from '@/api/inbound'
...
const handleSelectPurchase = async (row: any) => {
  try {
    await ElMessageBox.confirm(
      `确认根据采购申请表【${row.requestNo}】创建外转入库单吗？`,
      '提示',
      { type: 'warning' }
    )
    await createTransferFromPurchase(row.id!)
    ElMessage.success('外转入库单创建成功')
